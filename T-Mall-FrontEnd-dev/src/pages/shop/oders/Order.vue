...
export default {
  ...
  methods: {
    detail(index, rows) {
      ...
      request.get('/product/order1/' + oNumber + '/' + shopId).then((res) => {
        if (res.code === '0') {
          this.products = []
    ...
    cancel(index, rows) {
      this.$confirm('此操作将取消该订单, 是否继续?', '提示', {
        ...
        .then(() => {
          let oNumber = rows[index].orderNumber
		  let shopId = this.shopId
          request.patch('/order/cancel/' + oNumber + '/' + shopId).then((res) => {
            if (res.code === '0') {
              this.$message({
                message: res.msg,
                type: 'success',
    ...
    send(index, rows) {
      this.$confirm('确认发货吗?', '提示', {
        ...
        .then(() => {
          let oNumber = rows[index].orderNumber
		  let shopId = this.shopId
          request.patch('/order/send/' + oNumber + '/' + shopId).then((res) => {
            if (res.code === '0') {
              this.$message({
                message: res.msg,
                type: 'success',
        ...
        .then((res) => {
          ...
          if (res.code === '0') {
            ...
            res.data.data.forEach((item, i) => {
              item.index = i + 1
              request
                .get('/user/order_number/' + item.orderNumber + '/' + shopId)
                .then((res) => {
