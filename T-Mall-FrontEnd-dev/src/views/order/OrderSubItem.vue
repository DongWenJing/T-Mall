  ...
  <el-collapse-item :name="cIndex">
    <template slot="title">
      <div style="width: 100%">
        <el-row type="flex" style="text-align: center">
          ...
          <el-col :span="5">{{ cOrder.orderNumberAll }}</el-col>
          <el-col :span="4">{{ cOrder.createTime }}</el-col>
          <el-col :span="4">{{ cOrder.money }}</el-col>
          <el-col :span="3">
            <el-tag
              size="mini"
              :type="
    ...
    <div class="detail-bd">
      ...
      >
        ...
        <el-col :span="6" style="line-height: 120px; font-size: 15px"
          >原价:￥{{ item.productPrice }} x {{ item.count }}</el-col
        >
        <el-col :span="6" style="line-height: 120px; font-size: 15px"
          >应付:￥{{ item.orderProductPrice }}</el-col
        >
      </el-row>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        payMoney: this.cOrder.money,
        orderNumber: this.cOrder.orderNumberAll,
		orderAmount: this.cOrder.orderAmount
      },
  ...
  methods: {
    ...
    cancelOrder() {
      let oNumber = this.cOrder.orderNumberAll
      request.patch('/order/cancel1/' + oNumber).then((res) => {
        if (res.code === '0') {
          this.$message({
            message: res.msg,
            type: 'success',
  ...
  created() {
    let oNumber = this.cOrder.orderNumberAll
	let oAmount = this.form.orderAmount
